...
project_state: "deterministic_for_planning"
related:
  constitution: "./AGENTS.md"
  plan_target: "./plan.v0.1.0.md"
---

# NetSec StateKit (`netsec-sk`) MVP Specification v0.5.0
...
2. Filter to `.tgz` and `.tar.gz` only.
3. Canonicalize to absolute normalized paths.
4. Process in lexical ascending order of canonical path.
...
- Non-archive files are skipped and logged to `.netsec-state/ingest.ndjson` with `parse_error_fatal` + note `unsupported_extension`.
...
- Usage: `netsec-sk topology [--repo <path>] [--env <env_id>]`
- Stdout lines:
...
- Interactive shell supports: `help`, `help <command>`, `env list`, `env create`, `ingest`, `export`, `devices`, `panorama`, `show device`, `show panorama`, `topology`, `exit`, `quit`.
...
7. Extraction cleanup follows Section 6.3.
8. TSF identity and dedupe follow Sections 6.4 and 6.5.
9. Parse result classification follows Section 6.6 boundaries.
10. State unchanged detection uses Section 7.4 hash rules.
11. Each state-changing TSF creates exactly one commit with Section 10.1 allowlist.
12. Commit message format matches Section 10.2.
13. Per-environment exports are generated with exact contracts in Section 8.
14. Topology inference is IPv4-only and VR-aware; no cross-env edges.
15. `open` interactive shell prompt/commands match Section 9.4.
16. Help output exists for all commands and `help <command>` usage/examples.
17. RDNS behavior follows Section 6.8 and writes status fields deterministically.
18. No files under `.netsec-state/` are committed.
...
- Decision: canonical spec is `docs/spec.v0.5.0.md` with front matter version `0.5.0`.
...
- Options considered:
  - input order only.
  - lexical canonical path ordering.
- Decision: Section 3.4 lexical canonical ordering.
- Consequences: deterministic batch processing across platforms.
...
- Decision: Section 6.6 boundary rules.
...
As of 2026-02-08, unresolved blocker count is 0.
...
Determinism status: `READY_FOR_PLAN` (effective 2026-02-08).
